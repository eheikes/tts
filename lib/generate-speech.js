...
exports.generateSpeech = (ctx, task) => {
  ...
  ctx.opts = Object.assign({}, {
    ...
    'private-key': ctx.args.privateKey,
    'project-file': ctx.args.projectFile,
    'project-id': ctx.args.projectId,
    region: ctx.args.region || 'us-east-1',
    'sample-rate': ctx.args.sampleRate,
    'secret-key': ctx.args.secretKey,
    type: ctx.args.type || 'text',
    voice: ctx.args.voice || (ctx.service === 'gcp' ? 'en-US-Standard-C' : 'Joanna')
  }, ctx.args)
  ...
  if (ctx.args.privateKeyFile) {
    debug('generateSpeech')(`Reading private key from ${ctx.args.privateKeyFile}`)
    ctx.opts['private-key'] = fs.readFileSync(ctx.args.privateKeyFile, 'utf8')
  }
